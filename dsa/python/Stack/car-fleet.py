...
# reach the mile target. You are given two integer array position and speed,
...
# car fleet is the minimum speed of any car in the fleet. If a car catches up
# to a car fleet at the mile target, it will still be considered as part of
# the car fleet.
    ...
    def carFleet(
        ...
        for i in range(len(pairs) - 1, -1, -1):
            if stack:  # [ ..., 2, 1]
                ...
                t2 = (target - pairs[i][0]) / pairs[i][1]
                t1 = (target - sval[0]) / sval[1]
...
if __name__ == '__main__':
    ...
    print(res.carFleet(target=10, position=[3], speed=[3]))
    print(res.carFleet(target=100, position=[0, 2, 4], speed=[4, 2, 1]))
    print(res.carFleet(target=10, position=[0, 4, 2], speed=[2, 1, 3]))
