...
# return -1. int Rear() Gets the last item from the queue. If the queue is
# empty, return -1. boolean enQueue(int value) Inserts an element into the
# circular queue. Return true if the operation is successful. boolean deQueue()
# Deletes an element from the circular queue. Return true if the operation is
# successful. boolean isEmpty() Checks whether the circular queue is empty or
# not. boolean isFull() Checks whether the circular queue is full or not. You
# must solve the problem without using the built-in queue data structure in
...
class MyCircularQueue:
    ...
    def deQueue(self) -> bool:
        if self.isEmpty():
            return False
        p, n = self.left, self.left.next.next
        p.next, n.prev = n, p
        self.space += 1
        return True
